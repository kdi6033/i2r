...
| `command` (`c`) | 예제 | 상세 설명 |
|----------------|-------|-----------|
| **`df`** | `{ "c": "df", "m": "D8:13:2A:C3:E7:68", "fileName": "i2r-03.ino.bin" }` | 지정 보드에 펌웨어를 OTA 방식으로 다운로드 및 업데이트 |
| **`si`** | `{ "c": "si", "ssid": "i2r_wifi", "password": "00000000", "email": "user@example.com", "mqttBroker": "mqtt.i2r.link" }` | 보드 네트워크 및 MQTT 설정 저장 |
| **`so`** | `{ "c": "so", "m": "A0:B7:65:CD:4D:34", "n": 1, "v": 1 }` | 특정 보드의 출력 핀을 ON/OFF 제어 |
| **`gs`** | `{ "c": "gs", "m": "EC:64:C9:43:E8:B8" }` | 보드의 상태 요청 (응답: 온도, 습도, 입출력 배열 등) |
| **`sch`** | `{ "c": "sch", "m": "A0:B7:65:CD:4D:34", "o": "insert", "n": 0, "start": 540, "end": 620, "rm": 0, "dw": [1,2,3,4,5] }` | 월~금 오전 9:00~10:20 동안 0번 출력 ON |
| **`bio`** | `{ "c": "bio", "o": "save", "m": "A0:...", "n": 0, "ps": [{ "m": "D4:...", "n": 1, "v": 1 }, { "m": "B0:...", "n": 1, "v": 0 }] }` | 입력 상태에 따라 다른 장치의 출력을 자동 제어 |
| **`bs`** | `{ "c": "bs", "m": "A0:...", "o": "save", "t": "temp", "h": 28, "l": 27, "ps": [{ "m": "D4:...", "n": 0, "v": 1 }, { "m": "D4:...", "n": 0, "v": 0 }] }` | 센서 값이 상한 이상이면 ON, 하한 이하이면 OFF |
| **`ti`** | `{ "c": "ti", "light": 120 }` | 외부 센서(RP2040 등)에서 조도값 전송 |
