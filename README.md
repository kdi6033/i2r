...
| **`schedule_output`** (`sch`) | ```json<br>{ "c": "sch", "m": "A0:B7:65:CD:4D:34", "o": "insert", "n": 0, "start": 540, "end": 620, "rm": 0, "dw": [1,2,3,4,5] }<br>```출력핀 0번을 월~금 오전 9:00~10:20까지 자동으로 ON 되도록 예약합니다. `rm=0`은 매일 반복, `dw=[1~5]`는 월~금을 의미합니다. |
| **`bind_input_output`** (`bio`) | ```json<br>{ "c": "bio", "o": "save", "m": "A0:B7:65:CD:4D:34", "n": 0, "ps": [ { "m": "D4:8A:FC:B5:30:10", "n": 1, "v": 1 }, { "m": "B0:A7:32:1D:B3:B8", "n": 1, "v": 0 } ] }<br>```입력 포트(n)가 ON이 되면 지정된 출력 포트를 ON, OFF가 되면 다른 출력 포트를 OFF합니다. 입력-출력 간 연동 제어를 구성할 수 있습니다. |
| **`bind_sensor`** (`bs`) | ```json<br>{ "c": "bs", "m": "A0:B7:65:CD:4D:34", "o": "save", "t": "temp", "h": 28, "l": 27, "ps": [ { "m": "D4:8A:FC:B5:30:10", "n": 0, "v": 1 }, { "m": "D4:8A:FC:B5:30:10", "n": 0, "v": 0 } ] }<br>```온도 센서 값이 28도 초과 시 포트 ON, 27도 이하 시 OFF하는 자동 제어. `t`는 센서 종류(`temp`, `humi`, `light`)를 의미합니다. |
| **`touch_panel_input`** (`ti`) | ```json<br>{ "c": "ti", "light": 120 }<br>```RP2040 등의 외부 터치패널에서 측정한 조도 값을 IoT 보드에 전달하여 조건 제어에 활용할 수 있습니다. |
