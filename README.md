...
| **`set_info`** (`si`) | `{ "c": "si", "ssid": "i2r_wifi", "password": "00000000", "email": "user@example.com", "mqttBroker": "mqtt.i2r.link" }` | 보드가 네트워크에 접속하고 MQTT 서버에 연결하기 위해 필요한 정보를 설정합니다. SSID, 비밀번호, 이메일, 브로커 주소를 저장합니다. 이후 재시작 시 이 정보를 기반으로 자동 연결됩니다. |
| **`set_output`** (`so`) | `{ "c": "so", "m": "A0:B7:65:CD:4D:34", "n": 1, "v": 1 }` | 특정 보드의 지정된 출력 핀을 제어합니다. `n`은 핀 번호(0~3), `v`는 출력값으로 `true`는 ON, `false`는 OFF입니다. 릴레이, 모터, LED 등에 사용됩니다. |
| **`get_status`** (`gs`) | `{ "c": "gs", "m": "EC:64:C9:43:E8:B8" }` | 지정 보드의 현재 상태를 요청합니다. 응답으로는 온도, 습도, 입력포트 상태(`in`), 출력포트 상태(`out`), MAC, email 등의 정보가 포함됩니다. 실시간 모니터링 시 사용됩니다.<br><br>응답 예시:<br>`{"t": 3,"email": "kdi6033@gmail.com","m": "EC:64:C9:43:E8:B8","temp": 28.4,"humi": 38,"in": [0, 0, 0, 0],"out": [0, 0, 0, 0]}` |
| **`schedule_output`** (`sch`) | `{ "c": "sch", "m": "A0:B7:65:CD:4D:34", "o": "insert", "n": 0, "st": 540, "et": 620, "rm": 0, "dw": [1,2,3,4,5] }` | 540 = 9시 0분, 620 = 10시 20분<br>출력핀 0번을 월~금(15)에 매일 작동하도록 예약합니다. `rm`은 0 (매일 반복), `dw`는 요일을 배열로 지원합니다. |
| **`bind_input_output`** (`bio`) | `{ "c": "bio", "o": "save", "m": "A0:B7:65:CD:4D:34", "n": 0, "ps": [{ "m": "D4:8A:FC:B5:30:10", "n": 1, "v": 1 }, { "m": "B0:A7:32:1D:B3:B8", "n": 1, "v": 0 }] }` | 특정 입력 포트(n)의 상태(ON/OFF)에 따라 연동된 출력 포트들을 자동으로 제어합니다. 한 개 또는 여러 기기/포트를 묶어 입력에 반응하는 출력을 구성할 수 있습니다.<br><br>예: `"A0:B7:65:CD:4D:34"` 보드의 0번 포트가 ON이면 `"D4:8A:FC:B5:30:10"` 보드의 1번 포트가 ON되고, OFF이면 `"B0:A7:32:1D:B3:B8"` 보드의 1번 포트가 OFF됩니다. |
| **`bind_sensor`** (`bs`) | `{ "c": "bs", "m": "A0:B7:65:CD:4D:34", "o": "save", "t": "temp", "h": 28, "l": 27, "ps": [{ "m": "D4:8A:FC:B5:30:10", "n": 0, "v": 1 }, { "m": "D4:8A:FC:B5:30:10", "n": 0, "v": 0 }] }` | `"A0:B7:65:CD:4D:34"` 보드의 온도센서가 28도를 초과할 때는 `"D4:8A:FC:B5:30:10"` 보드의 0번 포트를 ON, 온도가 27도를 내려올 때는 동일 포트를 OFF합니다.<br><br>온도(`temp`), 습도(`humi`), 조도(`light`) 조건에 따라 출력 포트를 제어합니다.<br>예: 센서 값이 `high` 이상이면 ON, `low` 이하이면 OFF. 센서를 이용한 자동 환경 제어 구성에 사용됩니다. |
| **`touch_panel_input`** (`ti`) | `{ "c": "ti", "light": 120 }` | 외부 보드(RP2040 등)에서 측정된 센서값을 PLC 메인보드로 전달합니다.<br>예: 터치패널에서 조도 센서를 측정 후 조도값을 IoT 보드에 전달해 제어 조건으로 사용합니다. |
