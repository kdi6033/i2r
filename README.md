...
| **`get_status`** (`gs`) | ```{ "c": "gs", "m": "EC:64:C9:43:E8:B8" }```<br>지정 보드의 상태를 요청합니다. 응답으로 온도, 습도, 입력포트(`in`), 출력포트(`out`) 정보가 포함됩니다.<br><br>응답 예시:<br>`{"t":3,"email":"kdi6033@gmail.com","m":"EC:64:C9:43:E8:B8","temp":28.4,"humi":38,"in":[0,0,0,0],"out":[0,0,0,0]}` |
| **`schedule_output`** (`sch`) | ```{ "c": "sch", "m": "A0:B7:65:CD:4D:34", "o": "insert", "n": 0, "start": 540, "end": 620, "rm": 0, "dw": [1,2,3,4,5] }```<br>출력핀 0번을 월~금 오전 9:00~10:20까지 자동으로 ON 되도록 예약합니다. `rm=0`은 매일 반복, `dw=[1~5]`는 월~금을 의미합니다. |
| **`bind_input_output`** (`bio`) | ```{ "c": "bio", "o": "save", "m": "A0:B7:65:CD:4D:34", "n": 0, "ps": [ { "m": "D4:8A:FC:B5:30:10", "n": 1, "v": 1 }, { "m": "B0:A7:32:1D:B3:B8", "n": 1, "v": 0 } ] }```<br>입력 포트(n)가 ON이 되면 지정된 출력 포트를 ON, OFF가 되면 다른 출력 포트를 OFF합니다. 입력-출력 간 연동 제어를 구성할 수 있습니다. |
| **`bind_sensor`** (`bs`) | ```{ "c": "bs", "m": "A0:B7:65:CD:4D:34", "o": "save", "t": "temp", "h": 28, "l": 27, "ps": [ { "m": "D4:8A:FC:B5:30:10", "n": 0, "v": 1 }, { "m": "D4:8A:FC:B5:30:10", "n": 0, "v": 0 } ] }```<br>온도 센서 값이 28도 초과 시 포트 ON, 27도 이하 시 OFF하는 자동 제어. `t`는 센서 종류(`temp`, `humi`, `light`)를 의미합니다. |
