...
| `t`                    | type            | `gs`: 보드 종류 (예: `3` = i2r-03)<br>`bs`: 센서 타입 (`temp`, `humi`, `light`)    |
...
| `h`                    | sensorHighValue | 센서 동작의 상한 값 (예: 온도 28도)                                                |
| `l`                    | sensorLowValue  | 센서 동작의 하한 값 (예: 온도 26도)                                                 |
...
| `command` (`c`) | 예제          | 상세 설명           |
| --------------- | ------------- | ------------------ |
| **`download_firmware`** (`df`)  | `{ "c": "df", "m": "D8:13:2A:C3:E7:68", "fileName": "i2r-03.ino.bin" }`                                                                                        | 지정한 MAC 주소를 가진 보드에 인터넷을 통해 펌웨어를 다운로드합니다. 일반적으로 `.ino.bin` 또는 `.bin` 파일이 대상이며, OTA 업데이트를 통해 자동 적용됩니다.                                            |
| **`set_info`** (`si`)           | `{ "c": "si", "ssid": "i2r_wifi", "password": "00000000", "email": "user@example.com", "mqttBroker": "mqtt.i2r.link" }`                                        | 보드가 네트워크에 접속하고 MQTT 서버에 연결하기 위해 필요한 정보를 설정합니다. SSID, 비밀번호, 이메일, 브로커 주소를 저장합니다. 이후 재시작 시 이 정보를 기반으로 자동 연결됩니다.                                    |
| **`set_output`** (`so`)         | `{ "c": "so", "m": "A0:B7:65:CD:4D:34", "n": 1, "v":1 }`                                                                                                   | 특정 보드의 지정된 출력 핀을 제어합니다. `n`은 핀 번호(0\~3), `v`는 출력값으로 `true`는 ON, `false`는 OFF입니다. 릴레이, 모터, LED 등에 사용됩니다.                                         |
| **`get_status`** (`gs`)         | `{ "c": "gs", "m": "EC:64:C9:43:E8:B8" }`                                                                                                                      | 지정 보드의 현재 상태를 요청합니다. 응답으로는 온도, 습도, 입력포트 상태(`in`), 출력포트 상태(`out`), MAC, email 등의 정보가 포함됩니다. 실시간 모니터링 시 사용됩니다.    <br> 응답 예시: `{"t": 3,"email": "kdi6033@gmail.com","m": "EC:64:C9:43:E8:B8","temp": 28.4,"humi": 38,"in": [0, 0, 0, 0],"out": [0, 0, 0, 0]} `    |
| **`schedule_output`** (`sch`)   | `{"c": "sch","m": "A0:B7:65:CD:4D:34","o": "insert","n": 0,"st": 540,"et": 620,"rm": 0,"dw": [1,2,3,4,5]}`                                     |  540 = 9시 0분, 620 = 10시 20분<br>출력핀 0번을 월~금(15)에 매일 작동하도록 예약합니다. rm은 0 (매일), dw는 요일을 배열로 지원  |
| **`bind_input_output`** (`bio`) | `{"c": "bio","o": "save","m": "A0:B7:65:CD:4D:34","n": 0,"ps": [{ "m": "D4:8A:FC:B5:30:10", "n": 1, "v": 1},{ "m": "B0:A7:32:1D:B3:B8", "n": 1, "v":0} ]}`   | 특정 입력 포트(n)의 상태(ON/OFF)에 따라 연동된 출력 포트들을 자동으로 제어합니다. 한 개 또는 여러 기기/포트를 묶어 입력에 반응하는 출력을 구성할 수 있습니다.  "A0:B7:65:CD:4D:34" 0번 포트가 on 이 되면 "D4:8A:FC:B5:30:10" 보드의 1번포트가 on이 되고 "A0:B7:65:CD:4D:34" 0번 포트가 off가 되면 ""B0:A7:32:1D:B3:B8" 보드의 1번포트가 off 된다.     |
| **`bind_sensor`** (`bs`)        | `{"c":"bs","m":"A0:B7:65:CD:4D:34","o":"save","t":"temp","h":28,"l":27,"ps":[{"m":"D4:8A:FC:B5:30:10","n":0,"v":1},{"m":"D4:8A:FC:B5:30:10","n":0,"v":0}]}` | "A0:B7:65:CD:4D:34" 보드의 온도센서가 28도를 초과할 때는  "D4:8A:FC:B5:30:10" 보드의 0번포트를 on 하고 온도가 27도를 내려 올 떄는 "D4:8A:FC:B5:30:10" 보드의 0번포트를 off 한다. <br> 온도(`temp`), 습도(`humi`), 조도(`light`) 조건에 따라 출력 포트를 제어합니다. <br>예: 온도가 `high` 이상이면 출력 ON, `low` 이하이면 OFF.<br>센서 조건을 통해 환경 제어를 자동화할 수 있습니다. |
| **`touch_panel_input`** (`ti`)       | `{ "c": "ti", "light": 120 }`  | 외부 보드(RP2040 등)에서 측정된 센서값을 PLC 메인보드로 전달합니다. <br>예: 터치패널에서 조도 센서를 측정 후 조도값을 IoT 보드에 전달해 제어 조건으로 사용합니다.   |
...
|----------------|-------------------|
| `df` | `{ "c": "df", "m": "D8:13:2A:C3:E7:68", "fileName": "i2r-03.ino.bin" }`<br>지정 MAC 보드에 OTA 방식으로 펌웨어를 다운로드하고 자동 업데이트합니다. |
| `si` | `{ "c": "si", "ssid": "i2r_wifi", "password": "00000000", "email": "user@example.com", "mqttBroker": "mqtt.i2r.link" }`<br>보드에 네트워크 정보와 MQTT 브로커 설정을 저장합니다. 재시작 시 자동 연결됩니다. |
| `so` | `{ "c": "so", "m": "A0:B7:65:CD:4D:34", "n": 1, "v": 1 }`<br>특정 보드의 `n`번 출력 핀을 ON(`true`) 또는 OFF(`false`)로 제어합니다. |
| `gs` | `{ "c": "gs", "m": "EC:64:C9:43:E8:B8" }`<br>보드의 상태를 요청합니다. 응답에는 온도, 습도, `in`, `out`, `mac`, `email` 등이 포함됩니다. |
| `sch` | `{ "c": "sch", "m": "A0:B7:65:CD:4D:34", "o": "insert", "n": 0, "start": 540, "end": 620, "rm": 0, "dw": [1,2,3,4,5] }`<br>0번 출력 핀을 월~금 오전 9:00~10:20 동안 자동 ON 설정합니다. `rm=0`은 매일 반복, `dw`는 반복 요일입니다. |
| `bio` | `{ "c": "bio", "o": "save", "m": "A0:...", "n": 0, "ps": [{ "m": "D4:...", "n": 1, "v": 1 }, { "m": "B0:...", "n": 1, "v": 0 }] }`<br>입력 포트 `n`의 상태에 따라 여러 출력 포트들을 연동 제어합니다. 하나의 입력이 여러 출력을 제어할 수 있습니다. |
| `bs` | `{ "c": "bs", "m": "A0:...", "o": "save", "t": "temp", "h": 28, "l": 27, "ps": [{ "m": "D4:...", "n": 0, "v": 1 }, { "m": "D4:...", "n": 0, "v": 0 }] }`<br>센서 값(`temp`)이 28도 초과 시 첫 번째 출력 ON, 27도 이하 시 두 번째 출력 OFF 동작 수행. 다양한 센서 조건(`temp`, `humi`, `light`) 제어에 사용됩니다. |
| `ti` | `{ "c": "ti", "light": 120 }`<br>외부 보드(RP2040 등)에서 측정된 조도 값을 메인 보드에 전달하여 자동 제어의 조건으로 사용합니다. |
