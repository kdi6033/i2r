...
| `type`    | type      | 센서 유형 (`temp`, `humi`, `light`, ...) |
...
| `get_status`        | `gs`             | 보드 상태 요청 (온도, 습도, in/out 등) <br> 기능: 기기의 현재 상태 요청 {"c": "gs","m": "EC:64:C9:43:E8:B8"} <br> {"c": "gs","m": "EC:64:C9:43:E8:B8"} <br> 응답 예시: {"type": 3,"email": "kdi6033@gmail.com","mac": "EC:64:C9:43:E8:B8","temp": 28.4,"humi": 38,"in": [0, 0, 0, 0],"out": [0, 0, 0, 0]} |
...
## 🔁 bind_sensor (`bs`) 명령 세부 설명

`sensor` 관련 제어는 모두 `c: "bs"` 하나의 명령으로 통합되며,  
`type` 필드로 센서 종류를 구분합니다.
